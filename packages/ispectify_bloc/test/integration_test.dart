...
void main() {
  group(
    'BLoC integration: observer -> logger -> history',
    () {
      ...
      test('create + event + transition + state + close flow is captured',
        ...
        expect(
          find('create'),
          isNotNull,
          reason: 'onCreate should hit history',
        );
        ...
        final stateLogs = logger.history.where(
          (r) =>
              r.additionalData?[TraceKeys.category] == TraceCategoryIds.state,
        );
        expect(stateLogs, isNotEmpty);
      });

      test('correlation id links the full synchronous event lifecycle',
          () async {
        final bloc = _CounterBloc()..add('inc');
        await bloc.stream.firstWhere((s) => s == 1);
        await bloc.close();

        final eventLog = logger.history.firstWhere(
        ...
        for (final operation in const <String>[
          ...
          final lifecycleLog = logger.history.firstWhere(
            (r) => r.additionalData?[TraceKeys.operation] == operation,
          );
          expect(
            lifecycleLog.additionalData?[TraceKeys.correlationId],
            equals(eventCid),
          );
        }
      });
