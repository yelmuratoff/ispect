...
void main() {
  group('copyClipboard redaction', () {
    ...
    testWidgets('Given a value containing a Bearer token, '
        'When copyClipboard is called with redact: true, '
        'Then the clipboard contains the masked form', (tester) async {
    ...
    testWidgets('Given a JSON string with a "password" field, '
        'When copyClipboard is called with redact: true, '
        'Then the password value is masked', (tester) async {
      await tester.pumpWidget(appShell(const SizedBox.shrink()));
      final BuildContext context = tester.element(find.byType(SizedBox));
    ...
    testWidgets('Given a URL with embedded credentials, '
        'When copyClipboard is called with redact: true, '
        'Then credentials are masked', (tester) async {
      await tester.pumpWidget(appShell(const SizedBox.shrink()));
      final BuildContext context = tester.element(find.byType(SizedBox));
    ...
    testWidgets('Given a value and redact: false, '
        'When copyClipboard is called, '
        'Then the value is copied verbatim', (tester) async {
      await tester.pumpWidget(appShell(const SizedBox.shrink()));
      final BuildContext context = tester.element(find.byType(SizedBox));
    ...
    testWidgets('prefers an explicit service over keys and global policy', (
      ...
      copyClipboard(
        context,
        value:
            '{"global_field":"GLOBAL_RAW","local_field":"LOCAL_RAW",'
    ...
    testWidgets('bounds explicit raw clipboard output by UTF-8 bytes', (
      ...
      copyClipboard(context, value: _asciiString(4 * 1024 * 1024));
      await tester.pumpAndSettle();
    ...
    testWidgets('honors a local clipboard byte budget', (tester) async {
      ...
      expect(LogExportOutput.utf8Length(clipboardText!), lessThanOrEqualTo(64));
      expect(clipboardText, endsWith(LogExportOutput.truncatedMarker));
    });
  });
}
