...
List<PropSpec> paragraphProps(
  ...
  if (!target.softWrap)
    (icon: Icons.wrap_text, subtitle: 'soft wrap', child: const Text('off')),
...
List<PropSpec> decorationProps(
  ...
  if (d.shape != BoxShape.rectangle)
    (icon: Icons.circle_outlined, subtitle: 'shape', child: Text(d.shape.name)),
  if (d.border != null)
...
List<PropSpec> _decorationImageProps(
  ...
  (
    ...
    child: Text(
      img.fit?.name ??
          (img.centerSlice == null ? BoxFit.scaleDown.name : BoxFit.fill.name),
    ),
  ),
  if (img.alignment != Alignment.center)
    (
      ...
      child: EllipsizedText(
        describeAlignment(img.alignment, decimalPlaces: decimalPlaces),
      ),
    ),
  if (img.repeat != ImageRepeat.noRepeat)
    (icon: Icons.repeat, subtitle: 'bg repeat', child: Text(img.repeat.name)),
...
List<PropSpec> flexProps(RenderFlex target, {int decimalPlaces = 1}) => [
  (
    icon: Icons.swap_horiz,
    subtitle: 'direction',
    child: Text(target.direction.name),
...
List<PropSpec> transformProps(RenderTransform target, {int decimalPlaces = 1}) {
...
List<PropSpec> editableProps(RenderEditable target) => [
  ...
  if (target.obscureText)
    (icon: Icons.password, subtitle: 'obscure', child: const Text('on')),
  if (target.readOnly)
    (icon: Icons.lock_outline, subtitle: 'read only', child: const Text('on')),
...
List<PropSpec> parentDataProps(RenderBox target, {int decimalPlaces = 1}) {
  final parentData = target.parentData;
  if (parentData is FlexParentData) {
    return [
      if (parentData.flex case final flex? when flex > 0)
        (icon: Icons.expand, subtitle: 'flex', child: Text('$flex')),
      if (parentData.flex case final flex? when flex > 0)
        (
          icon: Icons.fit_screen,
          subtitle: 'flex fit',
...
final List<_Rule> _propsRules = [
  ...
  _rule<RenderFlex>((b, dp) => flexProps(b, decimalPlaces: dp), wrapper: false),
  _rule<RenderWrap>((b, dp) => wrapProps(b, decimalPlaces: dp), wrapper: false),
  _rule<RenderImage>(
  ...
  _rule<RenderBackdropFilter>((b, _) => backdropFilterProps(b), wrapper: true),
];
