...
- **Method-colored network rows:** The request-method pill and the card's left accent bar are colored per method (GET/POST/PUT/DELETE/…) on a theme-aware light/dark palette, matching the HTTP composer; the light palette is tuned to stay distinct from the success/error/pending status colors.
- **Clearer expanded transaction card:** The request URL is neutral and each section shows a de-duplicated summary — payload size plus the response reason only when it adds detail beyond the status code, and request content type and size — without repeating the method, URL, duration, or status code already in the header or surfacing empty "No data" rows.
