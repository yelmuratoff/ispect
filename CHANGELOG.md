...
- **Persistent log type filters:** Reopening the settings sheet no longer rolls the log type toggles back to the values captured at app start, nor overwrites the host's stored settings with that stale snapshot. `Select All` now clears an active filter instead of leaving it in place, `Deselect All` blocks every displayed type including custom ones, and disabling one type no longer silently suppresses log types registered through `customLogTypes`. Disabled types are now hidden from the log viewer immediately rather than only from future capture, and they no longer appear in the filter chip list, the error and warning counters, or an active chip selection. Exported sessions report the hidden type count as `excludedLogTypeKeysCount`.
- **Relative timestamps everywhere:** The `Relative time` display toggle reached only the desktop log table. Mobile log cards and the grouped HTTP transaction rows on both layouts always rendered the absolute capture time, so with `Group HTTP` on the same list mixed both formats. Entries older than a day still fall back to the absolute time.
...
- **Kurdish localization:** ISpect's UI is now translated into Sorani (`ckb`) and Kurmanji (`ku`); both resolve automatically through `ISpectLocalization.supportedLocales`. Apps that display ISpect in these locales own their Material/Cupertino delegates, as with every other locale.  
  Thanks [@RekanAnwar](https://github.com/RekanAnwar)
