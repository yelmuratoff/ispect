...
2. **For each new comment, ask one question** - _would a competent reader miss anything if this comment were removed?_ If the answer is no, delete the comment.
3. **Tighten what remains.** A comment that stays should describe _why_, fit on one line where possible, and use the language's doc-comment syntax for public APIs only.
