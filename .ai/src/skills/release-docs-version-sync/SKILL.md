...
Separate the counter from its label with a dot - `7.1.0-dev.1`, `7.1.0-dev.2`, … `7.1.0-dev.10`. Semver compares a dot-separated numeric identifier as a number and an identifier containing letters as text, so the glued form `7.1.0-dev10` resolves _below_ `7.1.0-dev8` and consumers keep getting the older code.
