...
- `./bash/publish.sh` asks pub.dev what it already serves and blocks a version the resolver does not rank above the peak of the same `MAJOR.MINOR` line. If it blocks, the version is wrong — do not reach for `--skip-pub-version-check`.
