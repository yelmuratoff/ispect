...
For writing the system prompt of an autonomous agent - a subagent, a slash command that drives a multi-step workflow, an IDE coding agent, a CLI agent. Distinct from one-off prompts because the persona is loaded once and steers _every_ turn.
...
Where on the spectrum from _clarify-first_ to _finish-end-to-end_ should the agent sit? State it.
...
What the _closing_ message of every turn should look like. The CLI / IDE will strip ANSI but renders Markdown - write rules accordingly.
...
- **Tool rules in the negative only** ("don't use cat") - agent doesn't know what _to_ use. Always pair "don't X" with "do Y".
